...
- **上游基线**：`upstream/main` 的 `7d0e8f3`（2026-08-19）。
- **上游内容**：Windows 安装包 CI 根据变更路径决定是否额外构建 Nightly；手动候选构建仍验证两个渠道，普通 PR 不再上传应用目录，并新增渠道选择器及工作流契约测试。
- **冲突处理**：本次上游提交与本分支自定义改动没有文件重叠，合并无冲突；上一基线 `24795a0` 的情报整合结果保持不变。
- **完整性结论**：本次没有修改任务逻辑、配置、UI、模板、持久化、调度或一键启动脚本；清单中的全部自定义功能均保留。新增及受影响的 Python 验证套件共 47 个测试通过，既有桌面 Reactor 594 个测试通过记录继续有效。
