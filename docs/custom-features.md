...
`IntelligenceRoutine` 在主开关关闭或所有子类型关闭时，于打开情报页之前退出；普通野兽与火晶野兽模板集合保持隔离，并同时使用彩色和灰度证据。上游合并不得删除入口和循环内的重复开关检查、View 按钮区域限制及误触遮罩恢复。
...
- **已完成证据**：`modules/desktop -am test` 完整通过；当前 Surefire 报告合计 565 个测试，0 失败、0 错误、0 跳过，包含新增/修改测试与 `FxmlLoadabilityTest`。
