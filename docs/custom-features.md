...
| 领域 | 上游基线 | 本项目扩展 |
| --- | --- | --- |
| 打熊加入 | 按可用加入入口执行常规加入 | 六队列独立编队已保留；高级候选模型、策略和分域 TTL 已完成纯逻辑，高级候选扫描与 ROI 解析已接入 |
| 手动集结加入 | 没有独立的目标筛选加入任务 | 按目标或全部目标扫描绿色 Join，并受出征数和编队配置约束 |
| 医院治疗 | 没有本项目的医院任务 | 野外入口和状态机部分完成；批次止损与退出调度已接入，城内和野外入口模板、伤兵总数读取与纯计算状态机已接入，仅剩高级加速功能仍待完善 |
