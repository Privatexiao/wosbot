...
- **上游基线**：`upstream/main` 的 `24795a0`（2026-08-19）。
- **上游内容**：情报导航与部署、研究分类树遍历、仓库奖励、多个真实帧模板，以及测试素材隐私脱敏。
- **冲突处理**：`IntelligenceRoutine` 同时保留上游“已打开侧栏快照 + 部署前证据”流程和本项目“主开关/全部子类型零触屏关停、运行中关停、独立单帧行军读取”保护。
- **完整性结论**：打熊高级加入、医院、手动集结、情报安全、极地恶魔/联盟自动集结、行军单帧读取、中文本地化、紧凑数值、异常证据和一键启动脚本均保留；完整桌面 Reactor 共 594 个测试通过。
