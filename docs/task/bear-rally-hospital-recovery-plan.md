...
- [ ] 一张 `720x1280` 未缩放原始 PNG：打熊联盟集结列表，最好同时显示两个以上可加入卡片，以及成员数、集结容量、剩余容量和倒计时。
- [ ] 一张 `720x1280` 未缩放原始 PNG：野外快捷医院图标可见的完整 WORLD 画面，用于复核现有入口模板和点击后页面转换。
- [ ] 一张 `720x1280` 未缩放原始 PNG：点击野外快捷医院图标后进入的完整治疗界面，用于确认入口结果及伤兵数、数量框和治疗时间区域。
- [ ] 一张 `720x1280` 未缩放原始 PNG：城内医院建筑可见的完整 HOME 画面。
- [ ] 一张 `720x1280` 未缩放原始 PNG：点击城内医院后进入的完整治疗界面。
- [ ] 一张 `720x1280` 未缩放原始 PNG：治疗开始并请求联盟帮助后的页面，需显示帮助状态或剩余倒计时。
- [ ] 说明游戏界面语言（中文或英文）。玩家名和联盟名可以脱敏，但不得裁剪、缩放或遮挡数值及按钮区域。

### 收到素材后的实施任务

- [ ] 标注并验证打熊卡片 ROI，使用同一帧解析发起人、成员数、总容量、剩余容量和倒计时。
- [ ] 先确认卡片字段语义，分别建模当前成员数、最大成员数、当前兵量、集结总容量和剩余容量；禁止用一组 `current/max` 同时代表人数和兵量。
- [ ] 恢复高级候选扫描、候选排序、策略门槛、狂热模式和活动实例级 TTL，禁止回退到固定假数据。
- [ ] 裁取并登记城内医院稳定模板，补齐 `TemplatesEnum`、`templates.properties` 和入口状态确认。
- [ ] 为打熊正反候选、城内医院存在/不存在及治疗页确认添加保存帧回归测试。
- [ ] 补齐医院任务重排策略，确保无入口、零伤兵、识别失败、治疗进行中和异常退出都不会立即重复执行。
- [ ] 检查每个医院配置项均有运行时读取、范围校验、日志和测试；未实现的配置控件必须禁用并明确提示。
- [ ] 运行相关模块测试，并按自动测试、保存帧验证、实时账号日志三个证据等级记录结果。
- [ ] 同步更新 [`docs/custom-features.md`](../custom-features.md)，只有上述验收通过后才移除“安全停用”说明。

### 当前安全边界

在素材和保存帧测试完成前，高级打熊卡片解析及城内医院入口保持安全停用；普通打熊兼容路径和野外医院入口不受此待办影响。
...
| 高级打熊纯策略、紧凑数值解析 | 部分完成 | 已有实现和测试源码，但本轮环境尚未实际运行测试 |
| 高级打熊卡片扫描、狂热与 TTL 集成 | 安全停用 | 缺真实卡片 ROI；候选模型仍只有一组 `current/max`，TTL 尚未绑定 profile 与活动实例 |
| 手动集结安全检查 | 已修复，待实机确认 | 颜色识别失败会跳过，登记前检查队列满和同目标弹窗 |
| 野外医院入口与治疗状态机 | 部分完成 | 有入口/治疗按钮模板，但缺完整保存帧、输入回读、可靠批次计算和明确重排策略 |
| 城内医院入口 | 安全停用 | 缺城内建筑模板及页面转换证据 |
...
- 候选签名至少包含 profile、活动实例、规范化发起人、稳定卡片字段和倒计时桶；同名发起人的不同集结不得互相覆盖。
- TTL 只在部署成功得到页面证据后写入，活动实例切换或结束时清空，并设置最大条目数及处理系统时钟回拨。
- 部署前复用单次行军队列快照；部署后检查队列已满、同目标、Deploy 仍可见及页面转换，任何失败都不得计为成功。

#### 医院计算与调度

- 读取伤兵总数、治疗总时间或经验证的单兵时间，并实际通过 `HealBatchCalculator` 将批次限制在 `[1, woundedCount]`。
...
**状态：部分完成；解析器和策略骨架已存在，TTL 生命周期与完整测试仍待完成。**
...
**状态：部分实现但不可宣称完成，需补输入回读、可靠批次、调度和保存帧验证。**
