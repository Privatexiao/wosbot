...
- [ ] 恢复高级候选扫描、候选排序及 TTL 运行时接入；候选模型、策略门槛、狂热决策和活动实例级 TTL 纯逻辑已完成，禁止回退到固定假数据。
...
在素材和保存帧测试完成前，高级打熊卡片解析及医院交互保持安全停用；普通打熊兼容路径不受此待办影响。医院纯逻辑可继续完善，但 UI 和运行时必须同时阻止旧配置进入未经验证的视觉交互。
...
| 高级打熊卡片扫描与 TTL 集成 | 安全停用 | 缺真实卡片 ROI；分域 TTL 已完成纯逻辑，但尚未接入真实扫描和部署成功证据 |
| 手动集结安全检查 | 已修复，待实机确认 | 颜色识别失败会跳过，登记前检查队列满和同目标弹窗 |
| 野外医院入口与治疗状态机 | 纯逻辑部分完成，运行时安全停用 | 批次止损和退出调度已接入；完整保存帧校准前 UI 与运行时双重拦截，仍缺伤兵总数读取、输入回读及页面转换证据 |
