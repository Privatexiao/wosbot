 ...
 * Features:
 * 1. Live Scene Graph Listener: Automatically intercepts & translates any newly added UI component in real-time.
 * 2. Exact Dictionary Matching: 500+ pre-translated terms for instant zero-latency lookup.
 * 3. Smart Term Fallback: Automatically decomposes compound English phrases into constituent terms if missing.
 * 4. Idempotent Node Marker: Prevents double-translation and text corruption.
 * 5. Zero Upstream Conflicts: 100% detached from FXML files, perfect for Git pull/merge.
 */
public class I18nService {
    ...
    /**
     * Registers a live scene graph change listener.
     * Automatically translates any new UI node added anywhere in the Scene at runtime.
     */
    public static void registerAutoTranslation(Scene scene) {
        ...
        // Translate initial root
        translateNode(scene.getRoot());
        attachTreeListener(scene.getRoot());

        // Listen for root property changes (e.g. view replacements)
    ...
    private static void attachTreeListener(Node node) {
        if (node instanceof Parent parent) {
            try {
                ...
                // Ignore unmodifiable list listener restrictions on certain internal skins
            }
        }
    }
