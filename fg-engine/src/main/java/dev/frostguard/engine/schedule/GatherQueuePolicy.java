...
public final class GatherQueuePolicy {
    ...
    public static List<TpDailyTaskEnum> getPendingHigherPriorityMarchTasks(AccountDescriptor profile) {
        ...
        Boolean intelRecall = profile.getConfig(dev.frostguard.api.configs.ConfigurationKeyEnum.INTEL_RECALL_GATHER_TROOPS_BOOL, Boolean.class);
        boolean isIntelRecallEnabled = Boolean.TRUE.equals(intelRecall);

        List<TpDailyTaskEnum> pendingTasks = Arrays.stream(TpDailyTaskEnum.values())
                .filter(HIGH_PRIORITY_MARCH_TASKS::contains)
                .filter(task -> {
                    if (task == TpDailyTaskEnum.INTEL && !isIntelRecallEnabled) {
                        return false;
                    }
                    return isTaskPending(profile, task);
                })
