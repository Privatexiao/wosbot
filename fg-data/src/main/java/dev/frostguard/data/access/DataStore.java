...
public final class DataStore implements AutoCloseable {
	...
	private static Map<String, Object> resolveDatabaseOverrides(Map<String, Object> overrides) {
		if (overrides != null && overrides.containsKey("jakarta.persistence.jdbc.url")) {
			return overrides;
		}
		Map<String, Object> map = new java.util.HashMap<>(overrides == null ? Map.of() : overrides);
		java.nio.file.Path dbPath = java.nio.file.Paths.get("database.db").toAbsolutePath();
		if (!java.nio.file.Files.exists(dbPath) && dbPath.getParent() != null && "target".equals(dbPath.getParent().getFileName().toString())) {
			java.nio.file.Path parentDb = dbPath.getParent().getParent().resolve("database.db");
			if (java.nio.file.Files.exists(parentDb)) {
				dbPath = parentDb;
			}
		}
		map.put("jakarta.persistence.jdbc.url", "jdbc:sqlite:" + dbPath.toString().replace('\\', '/') + "?busy_timeout=1000&journal_mode=WAL");
		return map;
	}

	private DataStore(Map<String, Object> overrides) {
		try {
			this.emf = Persistence.createEntityManagerFactory(PERSISTENCE_UNIT, resolveDatabaseOverrides(overrides));
			DataSeeder.populate(this);
			LOG.info("Frostguard data store initialized with URL: {}", resolveDatabaseOverrides(overrides).get("jakarta.persistence.jdbc.url"));
