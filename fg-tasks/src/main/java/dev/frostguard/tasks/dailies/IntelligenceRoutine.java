...
private static final int MIN_STAMINA_REQUIRED_FLOOR = 10;
...
public IntelligenceRoutine(AccountDescriptor profile, TpDailyTaskEnum tpTask) {
	...
	protected void execute() {
		...
		// Auto-join remains active during Intel missions
		...
		while (processingTask) {
			...
			if (survivorCampsEnabled) {
				...
				TemplatesEnum[] survivorTemplates = new TemplatesEnum[] {
						TemplatesEnum.INTEL_SURVIVOR_GRAYSCALE_FC,
						TemplatesEnum.INTEL_SURVIVOR_GRAYSCALE
				};
				for (TemplatesEnum survivorTemplate : survivorTemplates) {
			...
			if (explorationsEnabled) {
				...
				TemplatesEnum[] journeyTemplates = new TemplatesEnum[] {
						TemplatesEnum.INTEL_JOURNEY_GRAYSCALE_FC,
						TemplatesEnum.INTEL_JOURNEY_GRAYSCALE
				};
				for (TemplatesEnum journeyTemplate : journeyTemplates) {
	...
	private static final SearchConfig INTEL_CARD_SEARCH_CONFIG = SearchConfig.builder()
			.withThreshold(50)
			.withMaxAttempts(2)
			.withDelay(200L)
			.build();
...
private boolean shouldProcessBeastsFlow() {
	...
	private boolean seekAndProcessGrayscale(TemplatesEnum template, Consumer<ImageSearchResultData> processMethod) {
		...
		ImageSearchResultData result = templateSearchHelper.locatePatternMono(template, INTEL_CARD_SEARCH_CONFIG);

		if (result != null && result.isFound()) {
			int y = result.getPoint().getY();
			if (y < 200 || y > 1050) {
				logWarning(routineLogIntelligenceLine("Ignoring out-of-bounds template match for " + template + " at Y=" + y));
				return false;
			}
			logInfo(routineLogIntelligenceLine("Grayscale template detected: " + template + " at " + result.getPoint() + " (score: " + result.getMatchPercentage() + ")"));
		...
		try {
			for (int i = 0; i < CommonGameAreas.MARCH_SLOTS_TOP_LEFT.length; i++) {
				...
				if (text != null) {
					...
					if (lower.contains("idle") || text.contains("空闲") || text.contains("闲")) {
						idleCount++;
					}
				}
			}
...
private void manageRescheduling(boolean anyIntelProcessed, boolean nonBeastIntelProcessed,
	...
	private LocalDateTime resolveMarchReturnWaitTimeFlow(MarchesAvailable marchesAvailable) {
		LocalDateTime maxCap = LocalDateTime.now().plusMinutes(3);
		if (marchesAvailable != null && marchesAvailable.rescheduleTo() != null
				&& marchesAvailable.rescheduleTo().isAfter(LocalDateTime.now())) {
			return marchesAvailable.rescheduleTo().isBefore(maxCap) ? marchesAvailable.rescheduleTo() : maxCap;
		}
	...
	private boolean handleBeastIntel() {
		...
		if (beastsEnabled && !(useFlag && beastMarchSent)) {
			logInfo(routineLogIntelligenceLine("Scanning for beasts using grayscale matching."));
			TemplatesEnum[] beast_screenings = new TemplatesEnum[] {
					TemplatesEnum.INTEL_BEAST_GRAYSCALE_FC,
					TemplatesEnum.INTEL_BEAST_GRAYSCALE_FC1,
					TemplatesEnum.INTEL_BEAST_GRAYSCALE
			};
	...
	private void clickIntelViewButton(String missionType) {
		ImageSearchResultData view = templateSearchHelper.locatePattern(
	...
	private void clickMapActionOrView(String missionType, TemplatesEnum actionTemplate, String actionName) {
		...
		if (action != null && action.isFound()) {
			logInfo(routineLogIntelligenceLine("Found " + actionName + " button on map via template at " + action.getPoint()));
			tapPoint(action.getPoint());
		} else {
	...
	private void deployIntelMarch(String missionType) {
		...
		if (deploy != null && deploy.isFound()) {
			logInfo(routineLogIntelligenceLine("Found Deploy button via template for " + missionType + " at " + deploy.getPoint()));
		...
		if (confirmDialog.isFound()) {
			logInfo(routineLogIntelligenceLine("Stamina or deployment confirmation dialog detected. Pressing confirm at (430, 810)."));
			tapPoint(new PointData(430, 810));
			sleepTask(800);
		}
	...
	private void handleSurvivor(ImageSearchResultData result) {
		...
		PointData markerPoint = result.getPoint();
		logInfo(routineLogIntelligenceLine("Tapping Intel survivor marker icon at " + markerPoint));
		tapPoint(markerPoint);
		sleepTask(1500);

		clickIntelViewButton("survivor");
		clickMapActionOrView("survivor", TemplatesEnum.INTEL_RESCUE, "Rescue");
		deployIntelMarch("survivor");
		survivorMissionsSincePause++;
		StaminaService.getServices().subtractStamina(profile.getId(), SURVIVOR_STAMINA_COST_VALUE);
		StatisticsService.obtain().addToCounter(profile, "Intel Survivors", 1);
	}

	private void handleJourney(ImageSearchResultData result) {
		PointData markerPoint = result.getPoint();
		logInfo(routineLogIntelligenceLine("Tapping Intel journey marker icon at " + markerPoint));
		tapPoint(markerPoint);
		sleepTask(1500);

		clickIntelViewButton("journey");
		clickMapActionOrView("journey", TemplatesEnum.INTEL_EXPLORE, "Explore");
		deployIntelMarch("journey");
	...
	private void handleBeast(ImageSearchResultData beast) {
		...
		PointData markerPoint = beast.getPoint();
		logInfo(routineLogIntelligenceLine("Tapping Intel beast marker icon at " + markerPoint));
		tapPoint(markerPoint);
		sleepTask(1500);

		clickIntelViewButton("beast");
		clickMapActionOrView("beast", TemplatesEnum.INTEL_ATTACK, "Attack");
		deployIntelMarch("beast");
		...
		staminaHelper.subtractStamina(10, false);
		logInfo(routineLogIntelligenceLine("Intel beast march deployed. Continuing loop for next Intel mission."));
	}
}
