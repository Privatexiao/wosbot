...
private boolean confirmMailMenuOpened() {
        ...
        if (!inMailMenu.isFound()) {
            logWarning(routineLogMailRewardsLine("Mail menu open verification template not matched, proceeding with mail tab checks."));
            return true;
        }
...
private boolean openUpMailMenu() {
        ...
        if (mailMenu.isFound()) {
            ...
            logInfo(routineLogMailRewardsLine("Mail menu icon template not matched. Using fallback tap at (650, 1180)."));
            tapPoint(new PointData(650, 1180));
        }
        sleepTask(1000);
