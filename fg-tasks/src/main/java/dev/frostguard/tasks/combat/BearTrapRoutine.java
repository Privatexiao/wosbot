...
private static final boolean DEFAULT_CALL_OWN_RALLY_VALUE = true;

private static final boolean DEFAULT_JOIN_RALLY_VALUE = true;

private static final boolean DEFAULT_USE_PETS_VALUE = false;

private static final boolean DEFAULT_RECALL_TROOPS_VALUE = false;

private static final int DEFAULT_FREE_MARCHES_FALLBACK_VALUE = 6;
...
private void manageJoinRallies(int freeMarches) {
        int maxLoop = Math.min(Math.max(freeMarches, 1), 6);
        int joinedCount = 0;
        ...
        for (int i = 0; i < maxLoop; i++) {
            ...
            if (!plusIcon.isFound()) {
                if (joinedCount == 0) {
                    ...
                    logInfo(routineLogBearTrapLine("No more joinable rallies detected after joining " + joinedCount + " rallies."));
                }
                break;
            }

            int selectedFlag = resolveNextJoinFlag();
            logInfo(routineLogBearTrapLine("Joining rally [" + (joinedCount + 1) + "/" + maxLoop + "] with flag #" + selectedFlag + " (rotation: " + joinFlags + ")"));
...
private List<Integer> decodeJoinFlags() {
        ConfigurationKeyEnum[] marchKeys = {
        ...
        for (ConfigurationKeyEnum key : marchKeys) {
            String value = profile.getConfig(key, String.class);
            if (value != null && !value.trim().isEmpty() && !"No Flag".equals(value.trim())) {
                try {
                    ...
                    logWarning(routineLogBearTrapLine("Invalid join flag value: " + value));
                }
            }
        }
