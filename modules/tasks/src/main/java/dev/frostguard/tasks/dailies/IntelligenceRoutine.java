	...
	protected void execute() {
		...
		if (!hasEnabledIntelMissionType()) {
			logWarning(routineLogIntelligenceLine(
					"All Intel task sub-types are disabled. Exiting before opening the sidebar or Intel screen."));
			processingTask = false;
			return;
		}
		...
		while (processingTask) {
			if (!Boolean.TRUE.equals(profile.getConfig(ConfigurationKeyEnum.INTEL_BOOL, Boolean.class))) {
				logInfo(routineLogIntelligenceLine(
						"Intel was disabled while the routine was running. Stopping before further interaction."));
				processingTask = false;
				break;
			}
...
private int countIdleMarchesFlow() {
		return countIdleMarchesFlow(marchHelper.readMarchQueueSinglePass());
}
